...
#[test]
fn init_on_existing_cell_returns_error() {
    let db = db();
    db.state_set("cell", "existing").unwrap();
    // init on an existing cell is an error
    assert!(db.state_init("cell", "should-not-overwrite").is_err());
    // Original value unchanged
    assert_eq!(db.state_read("cell").unwrap(), Some(Value::String("existing".into())));
}
